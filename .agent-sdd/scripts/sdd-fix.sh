...
# -----------------------------------------------------------------------------
# sdd-fix.sh
# Apply a quick fix to a task using Agent-SDD instruction
# -----------------------------------------------------------------------------
...
Arguments:
  task-id     The ID of the task to fix (e.g., EXEC-001)
...
echo "🔧 Applying fix to task: $TASK_ID"
...
if claude /sdd-fix "$TASK_ID"; then
    echo "✅ Fix completed successfully for task: $TASK_ID"
else
    echo "❌ Fix failed for task: $TASK_ID"
    exit 1
fi