...
# -----------------------------------------------------------------------------
# sdd-tweak.sh
# Apply a tweak to a task using Agent-SDD instruction
# -----------------------------------------------------------------------------
...
Arguments:
  task-id     The ID of the task to tweak (e.g., EXEC-001)
...
echo "⚙️ Applying tweak to task: $TASK_ID"
...
if claude /sdd-tweak "$TASK_ID"; then
    echo "✅ Tweak completed successfully for task: $TASK_ID"
else
    echo "❌ Tweak failed for task: $TASK_ID"
    exit 1
fi