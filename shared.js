...
async function selectProject(projectPath, name) {
  ...
  const report = await window.__API.scan_project(projectPath)
...
async function chooseBaseDir() {
  const base = await window.__API.select_base_dir()
  if (!base) return
  state.baseDir = base
  document.getElementById('basePath').textContent = base

  const list = await window.__API.list_child_directories(base)
